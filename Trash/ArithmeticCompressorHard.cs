...
{
    public class ArithmeticCompressorHard
    {
        ...
        public ArithmeticCompressorHard(Stream input, Stream output)
        {
            Input = input;
            Output = output;
        }
        ...
        /*public void Decode()
        {
            ...
            while (Input.Position < Input.Length)
            {
                ...
                {
                    bool isTrue = lastRange.IsLastTrue();
                    if (isTrue) Console.WriteLine($"Position: {Output.Position} || i={i}; k={k} || Range: {lastRange} || Address: {address}");
                    else
                    {
                        Dictionary<byte, Tuple<double, double>> rangeDictionary = GetRanges(lastRange);
                        lastRange = GetRange(rangeDictionary, address);
